...
func handle_write(request: Dictionary, context: Dictionary) -> Dictionary:
	...
	var allow_missing_autoloads: bool = bool(params.get("allow_missing_autoloads", false))
	var path_error: Dictionary = _validate_script_path(script_path, false, context, request_id)
	if not path_error.is_empty():
		return path_error
	...
	if allow_missing_preloads or allow_missing_autoloads:
		return _write_skipping_soft_errors(script_path, source, request_id, context, allow_missing_preloads, allow_missing_autoloads)
...
func _write_skipping_preload_errors(script_path: String, source: String, request_id: String, context: Dictionary) -> Dictionary:
	return _write_skipping_soft_errors(script_path, source, request_id, context, true, false)


func _write_skipping_soft_errors(script_path: String, source: String, request_id: String, context: Dictionary, skip_preloads: bool, skip_autoloads: bool) -> Dictionary:
	...
	if err != OK:
		var all_soft_errors: bool = not capture.entries.is_empty()
		for entry in capture.entries:
			var msg: String = _entry_message(entry).to_lower()
			var is_preload_error: bool = skip_preloads and (msg.contains("preload") or msg.contains("not found") or msg.contains("resource") or msg.contains("load"))
			var is_autoload_error: bool = skip_autoloads and msg.contains("identifier not found")
			if not (is_preload_error or is_autoload_error):
				all_soft_errors = false
				break
		if not all_soft_errors:
