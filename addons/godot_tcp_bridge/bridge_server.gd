...
func _handle_node_get(request: Dictionary) -> Dictionary:
	...
	var encoded: Dictionary = _encode_typed_value(node.get(property))
...
func _handle_node_set(request: Dictionary) -> Dictionary:
	...
	var decoded: Dictionary = _decode_typed_value(params.get("value"))
	...
	return _bridge_ok(body.get("request_id", ""), {
		"path": _logical_path(node),
		"property": property,
		"value": _encode_typed_value(node.get(property)),
	})
...
func _decode_typed_value(encoded: Variant) -> Dictionary:
...
func _encode_typed_value(value: Variant) -> Dictionary:
	match typeof(value):
		TYPE_NIL:
